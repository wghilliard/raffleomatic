...
import {
    ...
    parseRaceResultFromLine,
    Prize, PrizeAward, PrizeDescriptor,
    PrizeWinner,
    PrizeWinners, RaceNumber,
...
function getCandidates(drives: RaceResult[], prize: Prize, awarded: PrizeAward[], winners: RaceResult[], cars: Map<CarNumber, Car>, previousWinners: Map<RaceNumber, PrizeWinner[]>) {
...
function hasWonToyo(driver: string, previousWinners: Map<RaceNumber, PrizeWinner[]>) {
    ...
    const windowSize = Math.min(previousWinners.size - 1, 9);
    const windowStart = previousWinners.size - windowSize;
    for (let i = windowStart; i < previousWinners.size; i++) {
        for (let j = 0; j < previousWinners[i].length; j++) {
            const record = previousWinners.get(i)[j];
...
function draw(awarded: PrizeAward[], prizes: Prize[], drives: RaceResult[], cars: Map<CarNumber, Car>, previousWinners: Map<RaceNumber, PrizeWinner[]>): RoundResults {
...
const {lastRace, winners}: PrizeWinners = loadV1Winners("winners.csv");
...
while (unawarded.length > 0 && round < 5) {
    ...
    }: RoundResults = draw(awarded, unawarded, raceResults.drives, cars, winners);
